...
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fflex?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/flex/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/flex-xpack/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fflex?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/flex/)

The **xPack flex** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
platform specific archives.

## Automated install

The easiest (and recommended) way to install flex
is via **xpm** and the package available
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/flex`}><code>@xpack-dev-tools/flex</code></a> from
...
<code>xpacks/@xpack-dev-tools/flex</code>.
...
**flex** package into the project.

The command to install the **latest** available version of **flex** is:
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/flex@${customField('releaseVersion')} --verbose
`} </CodeBlock>
...
* identify the platform specific archive for the latest available version
  of flex, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/flex</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/flex/.content/bin</code>;
* add <code>@xpack-dev-tools/flex</code> to
  `package.json` as a
  development dependency; this associates a specific version of flex with
...
it is also possible to install flex only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/flex@latest --global --verbose
`} </CodeBlock>
...
In order to access the flex binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/flex/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/flex/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/flex --verbose
`} </CodeBlock>

To completely remove the package from the user global xPacks store:

<CodeBlock language="sh"> {
`xpm uninstall --global @xpack-dev-tools/flex --verbose
`} </CodeBlock>
...
For all platforms, the **xPack flex** binaries are released as portable
archives that can be installed in any location.

The archives can be downloaded from
the <a href={`https://github.com/xpack-dev-tools/flex-xpack/releases`}>GitHub Releases</a> pages.
...
The macOS versions of **xPack flex**
are packed as `.tar.gz` archives.
Download the latest version named like:

<ul>
<li><code>xpack-flex-{customField('xpackVersion')}-darwin-x64.tar.gz</code></li>
<li><code>xpack-flex-{customField('xpackVersion')}-darwin-arm64.tar.gz</code></li>
</ul>
...
the <code>~/Library/xPacks/flex</code> folder, for
example <code>/Users/ilg/Library/xPacks/flex/xpack-flex-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/flex
cd ~/Library/xPacks/flex

tar xvf ~/Downloads/xpack-flex-${customField('xpackVersion')}-darwin-x64.tar.gz
chmod -R -w xpack-flex-${customField('xpackVersion')}
`} </CodeBlock>
...
<ul>
<li><code>xpack-flex-{customField('xpackVersion')}-linux-x64.tar.gz</code></li>
<li><code>xpack-flex-{customField('xpackVersion')}-linux-arm64.tar.gz</code></li>
<li><code>xpack-flex-{customField('xpackVersion')}-linux-arm.tar.gz</code></li>
</ul>
...
example <code>/home/ilg/.local/xPacks/flex/xpack-flex-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/flex
cd ~/.local/xPacks/flex

tar xvf ~/Downloads/xpack-flex-${customField('xpackVersion')}-linux-x64.tar.gz
chmod -R -w xpack-flex-${customField('xpackVersion')}
`} </CodeBlock>
